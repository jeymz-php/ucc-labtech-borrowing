...
<head>
    <meta charset="utf-8">
    <title>{{ $borrowing->borrowing_code }} Receipt</title>
    <style>
        body { font-family: Arial, sans-serif; color: #111827; margin: 32px; }
        .head { display: flex; justify-content: space-between; border-bottom: 2px solid #166534; padding-bottom: 16px; }
        .title { color: #166534; }
...
<body>
    ...
    <div class="head">
        <div>
            <h1 class="title">UCC LabTech</h1>
            <div>Borrowing Acknowledgment Receipt</div>
        </div>
        <div>
            <strong>{{ $borrowing->borrowing_code }}</strong><br>
    ...
    <div class="grid">
        <div>
            ...
            <div>{{ $borrowing->borrower_identifier ?: $borrowing->borrower_email }}</div>
        </div>
        ...
        <div>
            <div class="label">Borrow Schedule</div>
    ...
    <table>
        ...
        <tbody>
            @foreach ($borrowing->items as $line)
                <tr>
                    <td>{{ $line->itemUnit->item->name }}</td>
                    <td>{{ $line->itemUnit->asset_number }}</td>
                    <td>{{ $line->condition_out ?: '—' }}</td>
                    <td>{{ $line->condition_in ?: '—' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div style="margin-top: 60px; display: flex; justify-content: space-between;">
        <div>_________________________<br>Borrower Signature</div>
        <div>_________________________<br>Lab Technician</div>
    </div>
</body>
</html>
